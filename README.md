...
The API test suite relies on a single global database connection (defined in `marine_experiments/api.py`). **Do not create any new connections in your code; always use the global connection. Do not close the global connection.**

The tasks involve both Python and SQL; as much as possible, **data processing should be completed using SQL**.

**For tasks 0-7, you must write your queries in the `marine_experiments/queries` directory.**
...
The very trustworthy government agency would like to find all subjects with the letter 'o' in their name.
...
The government agency would like to see a combined view of information about its (volunteered) subjects. Write a query that will return information about subjects and their species. Your query result should have the below columns only:
...
Only return the experiment types and species that have an average score of higher than 5.

#### Example output 

  type_name   | species_name |         avg         
--------------|--------------|---------------------
...
The agency realises that they got their local Orca's name wrong, instead of Flounder, their name is Derek - the agency understands that if they're going to experiment on Derek, they may as well get their name right. Write a query that updates this.
...
The agency would like to create an API which gives their employees easy access to the experiments via HTTP requests. A `GET` request to the `/experiment` endpoint should return a list of objects (see example below). Each object should contain the following information only:
...
Score should be expressed as a percentage rounded to 2 d.p. (e.g. 70.34); no percentage sign should be included. The percentage score should be calculated based on the maximum score for that type of experiment.
...
The `/experiment` route should accept two optional query parameters. The endpoint will allow the employees to search by experiment type and for experiments that score over a specific value.
